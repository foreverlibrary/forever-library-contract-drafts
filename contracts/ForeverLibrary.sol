...
contract ForeverLibrary is ERC721URIStorage, ERC2981, ReentrancyGuard {
    ...
    Counters.Counter private _tokenIdCounter = Counters.Counter(1); // Start at 1 to avoid zero-index issues
    ...
    function _removeTokenFromOwnerEnumeration(address from, uint256 tokenId) private {
        ...
        assembly { // Use inline assembly for gas-efficient pop operation
            let len := sload(_ownedTokens.slot)
            sstore(add(_ownedTokens.slot, len), 0)
            sstore(_ownedTokens.slot, sub(len, 1))
        }
    ...
    function getTokenMetadata(uint256 tokenId) external view returns (address creator, uint256 creationBlock, bytes32 metadataHash) {
        ...
        creationBlock = _creationBlocks[tokenId];
        metadataHash = _metadataHashes[tokenId];
        return (creator, creationBlock, metadataHash);
        require(_exists(tokenId), "Token does not exist");
